...
data$wkday<- weekdays(data$NDate, abbreviate = T)
#create 3 labels for Axis
xaxis<-c(1,nrow(data)/2,nrow(data))
xaxislabels<-c(strftime(min(data$NDate),"%a"),strftime(max(data$NDate),"%a"),strftime(max(data$NDate)+1,"%a"))
png("plot2.png") 
